...
## 可以部署到 GitHub Pages 嗎？

可以。這個專案是 static site，不需要 build step，也不需要伺服器程式。GitHub Pages 會用 HTTPS 提供頁面，符合手機瀏覽器開啟相機的基本需求。

請注意：真正執行 AR 前，必須先放入以下檔案，否則頁面會開啟，但 MindAR 或素材會載入失敗：
...
這個專案是 static site，不需要 build step。repository 根目錄已提供 `index.html` 自動跳轉到 `./ar-frame/`，因此你可以直接部署整個 repository。

### 建議方法：部署整個 repository

1. 將整個 repository commit 並 push 到 GitHub。
2. 到 GitHub repository 的 **Settings** → **Pages**。
3. 在 **Build and deployment** 選擇 **Deploy from a branch**。
4. Branch 選擇你的分支，例如 `main`，資料夾選 `/root`。
5. 部署後可開啟根網址，會自動跳轉到 AR 頁面：
...
### 另一種方法：只部署 `ar-frame` 內容

如果你想讓網址根目錄直接就是 AR 頁面，也可以把 `ar-frame/` 裡面的內容移到 repository 根目錄，再用 GitHub Pages 部署根目錄。
